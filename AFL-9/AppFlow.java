...
public class AppFlow {
    ...
    private void tampilkanKatalogProduk() {
        ...
        System.out.printf("%-4s %-20s %-15s %-12s %-16s %-12s %-8s %-10s\n",
                "No.", "Nama Produk", "Petani", "Kategori", "Expirtaion Date", "Lokasi", "Harga", "Stok");
        System.out.println("---------------------------------------------------------------------");
            ...
            if (user instanceof Petani) {
                ...
                if (!petani.getStatus()) {
                    ...
                    for (Produk produk : petani.getProdukList()) {
                        ...
                        System.out.printf("%-4d %-20s %-15s %-12s %-16s %-12s Rp%,-8d %-8d\n",
                                nomor++,
                                produk.getNama(),
            ...
            switch (pilih) {
                case 1:
                    ...
                    if (kuantitas > katalogProduk.get(pilihProduk).getStok()) {
                        ...
                        ItemKeranjang item = new ItemKeranjang(katalogProduk.get(pilihProduk), kuantitas,
                                currentUserPembeli);
    ...
    public void checkout() {
        ...
        System.out.println("Biaya Layanan: " + layanan);
    ...
    public void menuLaporan(User user) {
    ...
    public void lihatLaporanSaya(User user) {
        System.out.println("\n=== LAPORAN SAYA ===");

        System.out.printf("%-4s %-15s %-15s %-20s %-12s\n",
                "No", "Pelapor", "Terlapor", "Alasan", "Tanggal");

        int no = 1;

        for (Laporan l : admin.getLaporan()) {
            if (l.getPelapor().equals(user)) {
                System.out.printf("%-4d %-15s %-15s %-20s %-12s\n",
                        no++,
                        l.getPelapor().getNama(),
    ...
    public void menuAdmin() {
        ...
        do {
            ...
            System.out.println("3. event diskon");
...
//sitem laporan
        ...
        if (aksi == 1) {
            ...
            currentUserAdmin.hapusTerlapor(l.getTerlapor());
...
//sistem diskon
    public void eventdiskon() {
    System.out.println("=== EVENT DISKON ===");
    System.out.print("Input diskon dalam persen (0-100): ");
    int diskonPersen = s.nextInt();
    s.nextLine();
    ...
    if (diskonPersen == 0) {
        System.out.println("Diskon direset menjadi 0%");
    }

    int jumlahProdukDiupdate = 0;

    for (User u : userList) {
        if (u instanceof Petani) {
            Petani p = (Petani) u;
            for (Produk produk : p.getProdukList()) {
                produk.setDiskon(diskonPersen); 
                jumlahProdukDiupdate++;
            }
        }
    }

    System.out.println("Event diskon berhasil diterapkan!");
    System.out.println("Besaran diskon : " + diskonPersen + "%");
    System.out.println("Produk : " + jumlahProdukDiupdate + " produk");
}
}
