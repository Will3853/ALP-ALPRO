...
public class AppFlow {
    ...
    public void start() {
        ...
        userList.add(admin);
        admin.buatEvent("Flash Sale", 10, 10000);
        ...
        do {
            ...
            switch (pilihan) {
                ...
                case 9:
                    ...
                    break;
                case 10:
                    User user = (Pembeli) currentUserPembeli;
                    buatLaporanUI(user);
                    break;
                default:
                    System.out.println("Tidak ada dalam pilihan");
...
System.out.printf(
"%-4s %-20s %-15s %-12s %-16s %-12s %-12s %-8s %-20s %-12s\n",
"No.", "Nama Produk", "Petani", "Kategori", "Expiration", "Lokasi", "Harga", "Stok", "Event", "Harga Asli");
        System.out.println("-----------------------------------------------------------------------------------------------------------------");
        ...
        for (User user : userList) {
            if (user instanceof Petani) {
                ...
                for (Produk produk : petani.getProdukList()) {
                    ...
                    if (produk.getEvent() != null && produk.getEvent().isAktif()) {
                        eventName = produk.getEvent().getNamaEvent();
                        hargaAsli = String.valueOf(produk.getHargaAsli());
                    }

                    System.out.printf(
                        "%-4d %-20s %-15s %-12s %-16s %-12s Rp%-10d %-8d %-20s %-12s\n",
                        nomor++,
                        produk.getNama(),
                        produk.getNamaPetani(),
                        produk.getKategori(),
                        produk.getExpiredDate(),
                        petani.getLokasi(),
                        produk.getHarga(),
                        produk.getStok(),
                        eventName,
            ...
            switch (pilih) {
                case 1:
                    ...
                    if (kuantitas > katalogProduk.get(pilihProduk).getStok()) {
                        ...
                        System.out.println("Gagal: Kuantitas Tidak Boleh 0 atau Kurang");
                    } else {
                        boolean itemKetemu = false;
                        ItemKeranjang item = new ItemKeranjang(katalogProduk.get(pilihProduk), kuantitas, currentUserPembeli);
                        for (ItemKeranjang itemKeranjang : currentUserPembeli.getItemKeranjang()) {
                            if (itemKeranjang.getProduk().equals(item.getProduk())) {
                                itemKetemu = true;
                                itemKeranjang.tambahKuantitas(item.getKuantitas());
                            }
                        }
                        if (itemKetemu) {
    ...
    public void menuKeranjang() {
        ...
        do {
            ...
            System.out.println("1. Ganti Kuantitas Produk");
            ...
            switch (pilih) {
                case 1:
                    ...
                    int produk = s.nextInt() - 1;
                    if (produk < 0 || produk >= currentUserPembeli.getItemKeranjang().size()) {
                        System.out.println("Tidak ada di pilihan");
                        break;
                    }
                    System.out.print("Kuantitas Baru: ");
                    int kuantitas = s.nextInt();
                    if (kuantitas <= 0 || kuantitas > currentUserPembeli.getItemKeranjang().get(produk).getProduk().getStok()) {
                        System.out.println("Tidak boleh lebih dari stok");
                        break;
                    }
                    currentUserPembeli.getItemKeranjang().get(produk).setKuantitas(kuantitas);
                    System.out.println("Berhasil Terganti!");
                    break;
                ...
                case 2:
                    ...
                    currentUserPembeli.getItemKeranjang().remove(produk);      
    ...
    public void lihatKeranjang() {
        ...
        System.out.println("Total Keranjang: " + currentUserPembeli.hitungTotalKeranjang());
    }

    public void checkout() {
        currentUserPembeli.displayCheckout();
        if (currentUserPembeli.getItemKeranjang().isEmpty()) {
            return;
        }
        System.out.println("---------------------------------------------------------------------");
        System.out.println("Subtotal Produk: " + currentUserPembeli.hitungTotalKeranjang());
        System.out.println("Subtotal Pengiriman: " + currentUserPembeli.hitungTotalOngkir());
        int total = currentUserPembeli.hitungTotalKeranjang() + currentUserPembeli.hitungTotalOngkir();
        ...
        switch (pilih) {
            case 1:
                currentUserPembeli.checkout(userList, total);
                break;
            case 2:
    ...
    public void lihatSuggestionPembeli() {
        ...
        if (currentUserPembeli.getSuggestionList().isEmpty())
            return;
    ...
    public void ajukanSuggestionPembeli() {
        ...
        switch (pilihKategori) {
            case 1:
                kategori = Kategori.SAYUR;
                break;
            case 2:
                kategori = Kategori.BUAH;
                break;
            case 3:
                kategori = Kategori.DAGING;
                break;
            default:
    ...
    public void menuPetani() {
        ...
        do {
            ...
            System.out.println("8. Masukkan Produk ke Event");
            System.out.println("9. Lihat Event");
            System.out.print("Pilihan: ");
            int pilihan = s.nextInt();
            switch (pilihan) {
                case 1:
                        ...
                        case 1:
                            ...
                            switch (pilihanKategori) {
                                case 1:
                                    ...
                                    s.nextLine();
                                    Produk produk = new Produk(namaSayur, currentUserPetani.getNama(), Kategori.SAYUR,
                                            hargaSayur, stokSayur, expiredDate, currentUserPetani);
                                ...
                                case 2:
                                    ...
                                    s.nextLine();
                                    produk = new Produk(namaBuah, currentUserPetani.getNama(), Kategori.BUAH, hargaBuah,
                                            stokBuah, expiredDate, currentUserPetani);
                                    currentUserPetani.tambahProduk(produk);
                                    break;
                                case 3:
                                    ...
                                    s.nextLine();
                                    produk = new Produk(namaDaging, currentUserPetani.getNama(), Kategori.DAGING,
                                            hargaDaging, stokDaging, expiredDate, currentUserPetani);
                                    currentUserPetani.tambahProduk(produk);
                                    break;
                                default:
                                    System.out.println("Tidak ada dalam pilihan");
                        ...
                        case 2:
                            if (currentUserPetani.getProdukList().size() != 0) {
                                ...
                                for (Produk produk : currentUserPetani.getProdukList()) {
                                    System.out.println(nomor + ". " + produk.getNama() + " - " + produk.getHarga()
                                            + " - " + produk.getStok() + " - " + produk.getExpiredDate());
                                    nomor++;
                                }
                                System.out.print("Pilihan: ");
                ...
                case 6:
                    ...
                    break;
                case 7:
                    User user = (Petani) currentUserPetani;
                    buatLaporanUI(user);
                    break;
                case 8: 
                    daftarEventPetani();
    ...
    public void prosesSuggestion() {
        ...
        if (currentUserPetani.getSuggestionList().isEmpty())
            return;
    ...
    public void laporanPenjualan() {
        ...
        System.out.printf("%-4s %-20s %-12s %-8s %-10s %-8s %-8s %-8s\n",
                "No.", "Nama Produk", "Kategori", "Harga", "Stok", "Expiration Date", "Total Penjualan",
                "Total Stok Terjual");
        System.out.println(
                "--------------------------------------------------------------------------------------------------------------");
        for (Produk produk : currentUserPetani.getProdukList()) {
            System.out.printf("%-4d %-20s %-12s %-8d %-10d %-8s Rp%,10d Rp%,10d\n",
                    nomor,
                    produk.getNama(),
                    produk.getKategori(),
                    produk.getHarga(),
                    produk.getStok(),
                    produk.getExpiredDate(),
                    produk.getTotalPenjualan(),
                    produk.getTotalStokTerjual());
            nomor++;
        }
        System.out.println("Total Pendapatan: Rp. " + currentUserPetani.getTotalPendapatan());
    }

    

    public void buatLaporanUI(User user) {
        Scanner sc = new Scanner(System.in);

        System.out.println("\n=== BUAT LAPORAN ===");

        System.out.print("Masukkan nama user/petani yang dilaporkan: ");
        String namaTarget = sc.nextLine();

        User target = cariUser(namaTarget);

        if (target == null) {
            System.out.println("User tidak ditemukan!");
            return;
        } else if (!target.getStatus()) {
            System.out.println("User sudah diblokir!");
            return;
        } else if (target == user) {
            System.out.println("Tidak dapat melaporkan diri!");
            return;
        }

        System.out.println("Pilih alasan:");
        System.out.println("1. Penipuan");
        System.out.println("2. Produk rusak");
        System.out.println("3. Harga tidak wajar");
        System.out.println("4. Lainnya");
        System.out.print("Pilih: ");
        int alasan = sc.nextInt();
        sc.nextLine();
        ...
        switch (alasan) {
            case 1:
                ...
                break;
            case 2:
                alasanText = "Produk rusak";
                break;
            case 3:
                alasanText = "Harga tidak wajar";
                break;
            case 4:
                System.out.print("Tulis alasan: ");
                alasanText = sc.nextLine();
                break;
        }

        Laporan laporan = new Laporan(user, target, alasanText);
        admin.addLaporan(laporan);
    ...
    public void prosesLaporan() {
        ...
        System.out.println("\n=== DAFTAR LAPORAN ===");
        ArrayList<Laporan> daftarLaporan = currentUserAdmin.getLaporan();
        if (daftarLaporan.isEmpty()) {
            System.out.println("Daftar Kosong");
            return;
        }
        for (int i = 0; i < currentUserAdmin.getLaporan().size(); i++) {
            Laporan l = daftarLaporan.get(i);
            System.out.println((i + 1) + ". " + l.getPelapor().getNama() +
                    " -> " + l.getTerlapor().getNama() +
                    " | " + l.getAlasan() +
                    " | " + l.getStatus());
        }
//sitem laporan
        ...
        if (idx < 0 || idx >= daftarLaporan.size()) {
            System.out.println("Index tidak valid!");
            return;
        }

        Laporan l = daftarLaporan.get(idx);

        System.out.println("1. Terima (ban user)");
        System.out.println("2. Tolak");
        System.out.print("Pilih: ");
        int aksi = sc.nextInt();

        if (aksi == 1) {
            currentUserAdmin.changeStatus(l.getTerlapor());
            System.out.println("User dibanned!");
            currentUserAdmin.SelesaiTerlapor(admin);

        } else {
            System.out.println("Laporan ditolak!");
            l.setStatus(LaporanStatus.DITOLAK);
        }
    }
//sistem diskon
    public void menuEventAdmin() {
    int pilih;
    ...
    do {
        ...
        System.out.println("3. Mulai Event");
        System.out.println("4. Stop Event");
        ...
        switch (pilih) {
            case 1:
                ...
                int diskon = s.nextInt();
                if (diskon < 1) {
                    System.out.println("Tidak boleh 0 atau kurang!");
                    return;
                } else if (diskon > 100) {
                    System.out.println("Diskon tidak boleh melebihi 100!");
                    return;
                }


                System.out.print("Maks Diskon: ");
                int maks = s.nextInt();
                if (maks< 1) {
                    System.out.println("Tidak boleh 0 atau kurang!");
                    return;
                }
                admin.buatEvent(nama, diskon, maks);
                break;

            case 2:
                if (admin.getEventList().isEmpty()) {
                    System.out.println("Event Kosong!");
                    break;
                }
                int i = 1;
                for (Event e : admin.getEventList()) {
                    System.out.println(i++ + ". " + e.getNamaEvent() +
                        " | Aktif: " + e.isAktif());
                }
                break;
            ...
            case 3:
                if (admin.getEventList().isEmpty()) {
                    System.out.println("Even Kosong!");
                    break;
                }
                i = 1;
                for (Event e : admin.getEventList()) {
                ...
                int idx = s.nextInt() - 1;
                if (idx < 0 || idx >= admin.getEventList().size() ) {
                    System.out.println("Tidak ada dalam pilihan");
                    break;
                }

                admin.getEventList().get(idx).mulaiEvent();
                break;
            ...
            case 4:
                ...
                admin.getEventList().get(idx).stopEvent();
                break;
            case 5:
                break;
            default:
    ...
    public void daftarEventPetani() {
        System.out.println("=== EVENT TERSEDIA ===");
        ...
        int i = 1;
        for (Event e : admin.getEventList()) {
            System.out.println(i++ + ". " + e.getNamaEvent() + " | Diskon: " + e.getDiskon() +"% | Maks Diskon: Rp "+ e.getMaksDiskon() +" | Aktif: " + e.isAktif());
        }
        ...
        if (idx < 0 || idx >= admin.getEventList().size()) {
            System.out.println("Event tidak valid!");
            return;
        }
        if (admin.getEventList().get(idx).isAktif()) {
            System.out.println("Tidak bisa mengikuti event yang aktif");
            return;
        }

        Event event = admin.getEventList().get(idx);

        System.out.println("=== PRODUK ANDA ===");

        ArrayList<Produk> list = currentUserPetani.getProdukList();

        if (currentUserPetani.getProdukList().isEmpty()) {
            System.out.println("Produk anda kosong");
            return;
        }
        for (int j = 0; j < list.size(); j++) {
            System.out.println((j + 1) + ". " + list.get(j).getNama());
        }

        System.out.print("Pilih produk: ");
        int pIdx = s.nextInt() - 1;

        if (pIdx < 0 || pIdx >= list.size()) {
            System.out.println("Produk tidak valid!");
            return;
        }


        Produk produk = list.get(pIdx); 
        if (produk.getEvent() != null) {
            System.out.println("Produk sudah terdaftar di event: " + produk.getEvent().getNamaEvent());
            return;
        }
        produk.setEvent(event);
        event.daftarProduk(produk);
        System.out.println("Produk berhasil didaftarkan ke event " + event.getNamaEvent());
    }
    public void lihatEventPetani() {
        int pilihan;
            System.out.println("=== EVENT YANG DI IKUTI ===");

            boolean ada = false;

            ArrayList<Produk> produkEvent = new ArrayList<>();
            for (Event e : admin.getEventList()) {

                    for (Produk p : e.getProdukEvent()) {
                        if (p.getPetani().equals(currentUserPetani)) {
                            produkEvent.add(p);
                            System.out.println("- " + p.getNama() + " | Harga: " + p.getHargaDiskon());
                            ada = true;
                        }
                    }
            }
            ...
            if (!ada) {
                System.out.println("Kamu belum ikut event apapun");
                return;
            }
