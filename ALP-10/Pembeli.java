...
public class Pembeli extends User{
   ...
   public void terimaSuggestion(int i) {
    ...
    public void checkout(ArrayList<User> userList, int total) {
        ...
        if (user2 instanceof Petani) {
            ...
            for (ItemKeranjang item : keranjang) {
                if (item.getProduk().getNamaPetani().equals(petani.getNama()) && petani.getStatus()) {
    ...
    public void BeriUlasan(Produk p) {
        ...
        if (rating < 1 || rating > 5) {
            System.out.println("Beri Rating dari 1-5!");
            return;
        }
